...
export async function postViabilitys(MySQL: Pool, viabilitys: Viability[]): Promise<MySQLResponse> {
    let query = 'INSERT INTO viability(clientName, phoneNumber, email, postalCodeId, city, neighborhood, state, address, number, complement, type, condominio, block, apartment, unionNumber) VALUES';
    ...
    for (const viability of viabilitys) { 
        query += `(${escape(viability.clientName)}, ${escape(viability.phoneNumber)}, 
            ...
            ${escape(viability.type)}, ${escape(viability.condominio)}, ${escape(viability.block)}, ${escape(viability.apartment)}, ${escape(viability.unionNumber)}),`;
    }
