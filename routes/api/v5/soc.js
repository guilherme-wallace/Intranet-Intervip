...
router.get('/eventos', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const QUERY = `SELECT * FROM soc_wanguard_report ORDER BY data_evento DESC LIMIT 100`;
    database_1.LOCALHOST.query(QUERY, (error, results) => {
        if (error) {
...
router.post('/salvar', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    let { id, id_wanguard, data_evento, ip_interno, cliente_nome, cliente_id_ixc, login, trafego_upload, trafego_download, equipamento, analise, acao_tomada, observacoes, status, usuario_responsavel } = req.body;
    if (id) {
        const QUERY = `UPDATE soc_wanguard_report SET 
            equipamento = ?, analise_preliminar = ?, acao_tomada = ?, observacoes = ?, status = ?, login = ?
            WHERE id = ?`;
        database_1.LOCALHOST.query(QUERY, [equipamento, analise, acao_tomada, observacoes, status, login, id], (error) => {
            if (error)
                return res.status(500).json({ error: error.message });
    ...
    if (!cliente_id_ixc && ip_interno) {
        try {
            const infoIxc = yield consultarIxcPorIp(ip_interno);
            if (infoIxc && infoIxc.total > 0) {
                ...
                if (((_b = (_a = respCliente.data) === null || _a === void 0 ? void 0 : _a.registros) === null || _b === void 0 ? void 0 : _b.length) > 0) {
                    cliente_nome = respCliente.data.registros[0].razao;
                }
            }
        }
        catch (e) {
            console.error("Erro identificação automática:", e);
        }
    }
    const CHECK_ID_WANGUARD = `SELECT id FROM soc_wanguard_report WHERE id_wanguard = ? LIMIT 1`;
    database_1.LOCALHOST.query(CHECK_ID_WANGUARD, [id_wanguard], (err, idExists) => {
        if (err)
        ...
        database_1.LOCALHOST.query(CHECK_OPEN, [ip_interno, cliente_id_ixc], (err, results) => {
            if (err)
                return res.status(500).json({ error: err.message });
            if (results && results.length > 0) {
                ...
                const UPDATE_INC = `UPDATE soc_wanguard_report SET 
                                    ...
                                    WHERE id = ?`;
                database_1.LOCALHOST.query(UPDATE_INC, [trafego_upload, trafego_download, id_wanguard, regExistente.id], (e) => {
                    if (e)
...
router.delete('/excluir/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const QUERY = `DELETE FROM soc_wanguard_report WHERE id = ?`;
    database_1.LOCALHOST.query(QUERY, [id], (error) => {
        if (error) {
...
const consultarIxcPorIp = (ip) => __awaiter(void 0, void 0, void 0, function* () {
...
router.get('/buscar-cliente-ip/:ip', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { ip } = req.params;
    ...
    try {
        const respRad = yield axios_1.default.post(`${urlBase}/radusuarios`, {
            qtype: "radusuarios.ip",
            query: ip,
            oper: "=",
            rp: "1"
        ...
        const respCliente = yield axios_1.default.post(`${urlBase}/cliente`, {
            qtype: "cliente.id",
            query: idCliente,
            oper: "=",
            rp: "1"
...
router.get('/equipamentos-lista', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
router.get('/relatorio-consumo/:loginPPPoE', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const respId = yield axios_1.default.post(`${urlBase}/radusuarios`, {
        ...
        const respConsumo = yield axios_1.default.post(`${urlBase}/radusuarios_consumo_d`, {
...
const processarAlertaIxc = (idReportLocal, idClienteIxc) => __awaiter(void 0, void 0, void 0, function* () {
    if (!idClienteIxc)
        return;
    const urlBase = `${process.env.IXC_API_URL}/webservice/v1`;
    const headersListar = {
    ...
    try {
        const respGet = yield axios_1.default.post(`${urlBase}/cliente`, {
        ...
        if (alertaAtual.includes("uso incomum de internet")) {
            ...
            database_1.LOCALHOST.query(UPDATE_LOCAL, [idReportLocal], () => { });
            return;
        }
        ...
        yield axios_1.default.put(`${urlBase}/cliente/${idClienteIxc}`, clienteDados, { headers: headersEditar });
        const UPDATE_LOCAL = `UPDATE soc_wanguard_report SET alerta_ixc = 'Sim' WHERE id = ?`;
        database_1.LOCALHOST.query(UPDATE_LOCAL, [idReportLocal], (err) => {
            if (err)
                console.error("Erro ao atualizar flag alerta_ixc:", err);
            else
                console.log(`Alerta IXC atualizado com sucesso para o cliente ${idClienteIxc}`);
        });
    }
    catch (error) {
