...
exports.UsuariosDB = {
    buscarPorUsuario: (usuario) => {
        return new Promise((resolve, reject) => {
            ...
            database_1.LOCALHOST.query(QUERY, [usuario], (err, results) => {
                if (err)
                    reject(err);
    ...
    sincronizarUsuarioAD: (nome, usuario, senhaAberta, grupo) => __awaiter(void 0, void 0, void 0, function* () {
        const hash = yield bcrypt.hash(senhaAberta, saltRounds);
        return new Promise((resolve, reject) => {
            const QUERY = `
                ...
                ON DUPLICATE KEY UPDATE 
                    ...
                    ultimo_login = NOW()
            `;
            database_1.LOCALHOST.query(QUERY, [nome, usuario, hash, grupo], (err) => {
                if (err)
                    reject(err);
                resolve();
            });
    ...
    sincronizarIXC: (username) => __awaiter(void 0, void 0, void 0, function* () {
        //console.log(`\n[DEBUG IXC] Iniciando sincronização para o usuário: '${username}'`);
        const user = yield exports.UsuariosDB.buscarPorUsuario(username);
        if (!user) {
        ...
        try {
            ...
            if (response.data && response.data.registros && response.data.registros.length > 0) {
                ...
                yield new Promise((resolve, reject) => {
                    ...
                    database_1.LOCALHOST.query(QUERY, [idUsuarioIxc, idFuncionarioIxc, statusIxc, idGrupoIxc, user.id], (err, result) => {
                        if (err) {
